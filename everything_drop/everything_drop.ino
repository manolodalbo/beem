...
void setup() {
  ...
  dataFile = SD.open("not_dropping", FILE_WRITE);

    // If the file is available, write to it
  if (dataFile) {
    Serial.print("Data will be written to file: ");
    Serial.println("not_dropping.csv");
    writeDataToSDCard("yay writing")
    dataFile.close();
  } else {
...
void loop() {
  ...
  while(a>2.00) {
    drop += 1;
    dataFile = SD.open(String("drop_" + drop" + .csv"),FILE_WRITE)
    startCamera();
    printData();
    readWaterSensor();
    a = getYAcceleration();
  ...
  if (going == 1) {
    dataFile.close();
    dataFile = SD.open(String("not dropping.csv"))
    toRunWhileDecel();
    endCamera();
  }
}
...
float getYAcceleration() {
  ...
  writeDataToSD(String(GetTimeStamp() + "," + "a_x: " + event.acceleration.x));
  writeDataToSD(String(GetTimeStamp() + "," + "a_y: " + event.acceleration.y));
  writeDataToSD(String(GetTimeStamp() + "," + "a_z: " + event.acceleration.z));
